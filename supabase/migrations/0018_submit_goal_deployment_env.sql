-- 0018_submit_goal_deployment_env.sql
-- P2 drill D-P2-1 live defect fix. OWNER-APPLIED ONLY, to BOTH staging
-- and production (parity discipline).
--
-- Defect (found live, prod host, 2026-08-18 tick disp-92809):
-- submit_goal_decomposition (0010) inserts master_goals.environment as
-- the hardcoded literal 'staging'. 0017 widened the CHECKs and added
-- runtime_deployment, but this function was never re-pointed. On
-- staging the literal coincidentally matches; on production every
-- composed goal is born environment='staging' and the dispatcher's
-- deployment-equality invariant refuses the run (fail-closed - the
-- guard worked; the stamp was wrong).
--
-- Fix: EXACT copy of the 0010 function body with precisely three
-- deltas (the 0017 read_ssot_status delta pattern):
--   (a) declare v_env text;
--   (b) select environment into v_env from runtime_deployment 'self';
--   (c) the insert stamps coalesce(v_env, 'staging') instead of the
--       literal - an unseeded deployment still can never claim
--       production (0017 principle preserved).
-- Everything else byte-identical to 0010. Grants unchanged/re-stated.
