// Preston Supervisor - supervisor feed + notification logic. PURE,
// SDK-free, ported from the staging-verified reference implementation
// (preston-os feature/hermes-dashboard @ 1f97e87,
// apps/dashboard/src/lib/hermes/feed.ts). The wire shape mirrors
// preston_poll_events output; the SERVER stays authoritative for
// event ids, ordering (incl. SB-1 same-millisecond lifecycle
// ranking) and cursor semantics - this module only accumulates,
...
// `backfill` marks events absorbed while anchoring (first load, or a
// re-anchor after cursor_invalid): they are window HISTORY being
// replayed, never fresh news, and must not become notifications.
...
// Merge a page into the accumulated entries. Deterministic event ids
// are the dedup key: an id already present is dropped (a repeated
// page or an overlapping poll can never duplicate a row).
export function mergePage(
  ...
  return {
    entries:
      next.length > FEED_CAP ? next.slice(next.length - FEED_CAP) : next,
    added,
  };
}

// A submit-time rejection never entered the runtime: goal_id is null
// by design and the row must read differently from a runtime failure.
...
// localStorage key for the opaque cursor, scoped per PRESTON
// environment so a staging cursor can never be replayed against
// production data.
export function cursorStorageKey(environment: string): string {
  const env = /^[a-z_-]{1,32}$/.test(environment)
    ? environment
    : "unknown";
  return `hermes-preston.cursor.${env}`;
}
