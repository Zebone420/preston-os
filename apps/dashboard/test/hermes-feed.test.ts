...
import {
  ...
  type FeedEntry,
  type FeedEvent,
} from '../src/lib/hermes/feed';

// Hermes Supervisor Dashboard v0 - feed accumulation + notification
// derivation. Pins: deterministic-id dedup (a repeated or overlapping
// page can never duplicate), backfill suppression (window history is
// never a fresh notification - the cursor_invalid/re-anchor discipline
// depends on this), the notification class map (submit rejections stay
// distinct from runtime failures), and per-environment cursor keys.
...
describe('deriveNotifications', () => {
  ...
  it('maps the notification classes from authoritative kinds only', () => {
    ...
    const out = deriveNotifications(added);
    expect(out.map((n) => n.cls)).toEqual([
      'completed',
      'failed',
      'timed_out',
      'dead_lettered',
...
describe('cursorStorageKey', () => {
  it('scopes the cursor per environment', () => {
    expect(cursorStorageKey('staging')).toBe(
      'hermes.supervisor.cursor.staging',
    );
    expect(cursorStorageKey('production')).toBe(
      'hermes.supervisor.cursor.production',
    );
  });

  it('sanitizes unexpected environment strings', () => {
    expect(cursorStorageKey('')).toBe('hermes.supervisor.cursor.unknown');
    expect(cursorStorageKey('We!rd Env')).toBe(
      'hermes.supervisor.cursor.unknown',
    );
  });
});
