...
describe('GPT Actions REST surface', () => {
  ...
  it('openapi.json: 404 when disabled; bounded document with consequential flags and bridge auth URLs when enabled', async () => {
    ...
    expect(ops.map((o) => o.operationId).sort()).toEqual(
      ['cancelPrestonGoal', 'decidePrestonApproval', 'followUpPrestonGoal', 'getPrestonArtifact', 'getPrestonEvidence', 'getPrestonGoal', 'getPrestonJob', 'getPrestonOwnerView', 'getPrestonStatus', 'listPrestonApprovals', 'pollPrestonEvents', 'submitPrestonGoal'],
    );
    ...
    // Cancellation records an owner decision (goal + jobs stop being driven):
