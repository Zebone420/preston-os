...
// Thin orchestration service for Preston Super Brain v1.
// It deliberately exposes no generic tool execution surface. Providers can
// recall; memory writes must pass the Preston policy gate before reaching the
// sink. Preston Control remains the only authority/control plane.
...
export class PrestonBrain {
  ...
  async recall(query: BrainQuery): Promise<BrainContextItem[]> {
    ...
    if (!capability.allowed || !this.provider.capabilities.includes('recall')) {
      return [];
    }
  ...
  async remember(
    ...
    if (!this.memorySink) {
      return { ok: false, error: 'brain memory sink not configured' };
    }
    ...
    return this.memorySink.append({
      ...candidate,
      value: decision.sanitized_value,
    });
  }
}
