...
// Preston Super Brain v1 memory policy gate.
// Fail closed. Provider output is untrusted input. The Brain never converts
// remembered text into authorization or execution authority.
  ...
  const visit = (node: unknown): void => {
    if (typeof node === 'string') {
      if (SECRET_VALUE_PATTERNS.some((pattern) => pattern.test(node))) {
        result.secretValue = true;
      }
      if (AUTHORITY_PATTERNS.some((pattern) => pattern.test(node))) {
        result.authorityClaim = true;
      }
      return;
    }
...
export function evaluateMemoryCandidate(
  ...
  return {
    allowed: true,
    reasons: [],
    sanitized_value: redactSecrets(candidate.value),
  };
}
